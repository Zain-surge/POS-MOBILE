...
class CircularTimer extends StatefulWidget {
  ...
  final int maxMinutes; // Maximum minutes for full circle (default 60 minutes = 1 hour)
...
class _CircularTimerState extends State<CircularTimer> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalMinutes = math.min(_elapsed.inMinutes, widget.maxMinutes);
    final progress = totalMinutes / widget.maxMinutes;
    ...
    if (totalMinutes >= 45) {
      ...
      currentColor = Colors.yellow; // Orange from 30-45 minutes
    }

    // Show elapsed minutes only
...
class BackgroundCirclePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size canvasSize) {
    ...
    for (int i = 0; i < 4; i++) {
      final angle = (i * 15 / 60) * 2 * math.pi - math.pi / 2; // Every 15 minutes
    ...
    for (int i = 0; i < 12; i++) {
      if (i % 3 != 0) { // Skip the main hour markers
        final angle = (i * 5 / 60) * 2 * math.pi - math.pi / 2; // Every 5 minutes
        final markerStart = Offset(

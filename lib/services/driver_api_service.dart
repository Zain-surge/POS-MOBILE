...
class DriverApiService {
  ...
  static const String alternativeProxy = "https://corsproxy.io/?https://thevillage-backend.onrender.com";
  ...
  static Future<List<Map<String, dynamic>>> getOrdersWithDriver(String date) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final errorBody = jsonDecode(response.body);
        throw Exception(errorBody['error'] ?? 'Failed to fetch orders with driver');
      }
    } on SocketException {
      throw Exception('No internet connection');
    } on FormatException {

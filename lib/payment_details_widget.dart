...
class _PaymentWidgetState extends State<PaymentWidget> {
  ...
  void initState() {
    ...
    _selectedAmount = _discountedTotal; // Default to discounted total

    // If payment type is 'card', automatically enable custom amount mode
    // and pre-fill the amount with the discounted total.
  ...
  void _onAmountPaidChanged() {
    if (_isCustomAmountMode || widget.paymentType.toLowerCase() == 'card') {
  ...
  void _calculatePresetAmounts() {
    if (widget.paymentType.toLowerCase() == 'cash') {
      ...
      _presetAmounts = []; // No preset amounts for card payments
    }
  }
  ...
  void _calculateDiscountedTotal() {
    setState(() {
      ...
      // Adjust selected amount and custom mode based on payment type
      if (widget.paymentType.toLowerCase() == 'card') {
        _selectedAmount = _discountedTotal;
        _amountPaidController.text = _discountedTotal.toStringAsFixed(2);
        _isCustomAmountMode = true; // Stay in custom mode for card
      } else {
        _selectedAmount = _discountedTotal;
        _isCustomAmountMode = false;
      ...
      child: Column(
        children: [
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    // Conditional rendering of preset amount buttons or direct amount input
                    if (widget.paymentType.toLowerCase() == 'cash')
                      Column(
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                backgroundColor: _isCustomAmountMode && widget.paymentType.toLowerCase() == 'cash'
                                    ? Colors.grey[800]
                                    : Colors.black,
                                foregroundColor: Colors.white,
                                shape: RoundedRectangleBorder(
                    ...
                    // For card payments, just show the editable amount paid field directly
                    // This avoids the 'Other' button or preset options for cards.
                      Row(
          ...
          Padding(
            ...
            child: Column(
              children: [
                // Payment summary - Always show Amount Paid section for card,
                // or when in custom mode or when there's change for cash
                if (widget.paymentType.toLowerCase() == 'card' || _isCustomAmountMode || _selectedAmount >= _discountedTotal) ...[
                  // Amount Paid Row
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      ...
                      // Editable for card or when in custom mode for cash
                      ...
                      else // Display selected amount for cash if not in custom mode
                        Text(
                  ...
                  // Change Due for cash payments only
                  if (_selectedAmount > _discountedTotal && widget.paymentType.toLowerCase() == 'cash') ...[
                  ...
                  // For cash when amount paid is less than or equal to discounted total and not in custom mode
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text(
                    ...
                    Expanded(
                      child: MouseRegion(
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            // Validation for card payments: amount paid cannot be less than total
                            if (widget.paymentType.toLowerCase() == 'card' && _selectedAmount < _discountedTotal) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('For card payments, amount paid cannot be less than the total.'),
                                  backgroundColor: Colors.red,
                                ),
                              );
                              return;
                            }
                            // General validation for cash: amount paid must be sufficient
                            if (widget.paymentType.toLowerCase() == 'cash' && _selectedAmount < _discountedTotal) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Amount paid must be greater than or equal to the final total.'),
                                  backgroundColor: Colors.red,
                                ),
                              );
                              return;

...
class Order {
  ...
  String getDisplayStatusLabel() {
    ...
    final isDeliveryOrder = (orderSource.toLowerCase() == 'epos' && orderType.toLowerCase() == 'delivery') ||
        (orderSource.toLowerCase() == 'website' && orderType.toLowerCase() == 'delivery');

    if (isDeliveryOrder) {
      print('🚚 Delivery Order ${orderId}: status=${status}, driverId=${driverId}');

      // For delivery orders with driver assigned and ready status
      if ((status.toLowerCase() == 'ready' || status.toLowerCase() == 'green') && driverId != null) {
        print('🚚 Order ${orderId}: Showing "On Its Way" (status: ${status}, driver: ${driverId})');
        return 'On Its Way';
      }
      // For delivery orders that are completed
      else if (status.toLowerCase() == 'blue' || status.toLowerCase() == 'completed' || status.toLowerCase() == 'delivered') {
